  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/dev/check-secrets' && request.method === 'POST') {
      ...
      return json({
        ...
        GROQ_KEYS_B_len: env.GROQ_KEYS_B?.length || 0,
        GROQ_KEYS_C_len: env.GROQ_KEYS_C?.length || 0,
...
async function handleGetAudit(request, env, url) {
  const user = await verifyAuth(request, env)
  if (!user?.operator_id) return jsonError('No autenticado', 401, request)
  const rol = await getOperatorRole(user.operator_id, env)
  if (!rol || !['desarrollador', 'supervisor'].includes(rol)) return jsonError('Solo desarrolladores', 403, request)
...
async function handleGetAuditStats(request, env) {
  const user = await verifyAuth(request, env)
  if (!user?.operator_id) return jsonError('No autenticado', 401, request)
  const rol = await getOperatorRole(user.operator_id, env)
  if (!rol || !['desarrollador', 'supervisor'].includes(rol)) return jsonError('Solo desarrolladores', 403, request)
...
async function handleAnalyzeAudit(request, env) {
  const user = await verifyAuth(request, env)
  if (!user?.operator_id) return jsonError('No autenticado', 401, request)
  const rol = await getOperatorRole(user.operator_id, env)
  if (!rol || !['desarrollador', 'supervisor'].includes(rol)) return jsonError('Solo desarrolladores', 403, request)
