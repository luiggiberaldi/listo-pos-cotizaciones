...
async function handlePush(request, env, url) {
  ...
  if (route === 'subscribe' && request.method === 'POST') {
    ...
    if (!endpoint || !keys?.p256dh || !keys?.auth) {
      return jsonError('Suscripción incompleta');
    }
    ...
    if (!res.ok) return jsonError('Error al guardar suscripción', 500);
    return json({ ok: true });
  }
  ...
  if (route === 'send' && request.method === 'POST') {
    // Verificar que sea desde el propio servidor (internal auth) o supervisor
    const user = await verifyAuth(request, env);
    if (!user?.id) return jsonError('No autenticado', 401);
    ...
    if (!title || !message) return jsonError('Faltan title y message');
    ...
    if (!subsRes.ok) return jsonError('Error al obtener suscripciones', 500);
  ...
  return jsonError('Ruta push no encontrada', 404);
}
