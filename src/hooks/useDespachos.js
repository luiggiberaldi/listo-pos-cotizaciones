    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select(`
          ...
          transportista:transportistas!notas_despacho_transportista_id_fkey(id, nombre, rif, telefono, zona_cobertura, vehiculo, placa_chuto, placa_batea),
...
export function useCrearDespacho() {
  ...
  return useMutation({
    mutationFn: async ({ cotizacionId, notas = null, formaPago = null, transportistaId = null, fleteUsd = 0, numeroCotizacion, clienteNombre }) => {
      ...
      const res = await fetch(apiUrl('/api/despachos/crear'), {
        ...
        body: JSON.stringify({ cotizacionId, notas: notas || null, formaPago: formaPago || null, transportistaId: transportistaId || null, fleteUsd: Number(fleteUsd) || 0 }),
      })
