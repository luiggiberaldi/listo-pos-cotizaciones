...
export default function CajaCerradaOverlay({ cartCount, onOpenApertura, canOpen = true }) {
    return (
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className={`w-24 h-24 sm:w-32 sm:h-32 ${canOpen ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-slate-100 dark:bg-slate-800/50'} rounded-full flex items-center justify-center mb-6 shadow-sm border-4 border-white dark:border-slate-900`}>
                {canOpen
            ...
            {canOpen ? (
                ...
                <p className="text-slate-500 dark:text-slate-400 max-w-sm text-sm sm:text-base leading-relaxed">
                    No tienes permiso para abrir la caja. Solicita al administrador que aperture el turno.
                </p>
            )}
        </div>
    );
