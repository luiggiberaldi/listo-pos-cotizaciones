...
import { 
  X, Clipboard, FileText, Camera, Check, AlertTriangle, 
  Trash2, Plus, Sparkles, RefreshCw, UploadCloud, Eye, Info, CheckCircle2 
} from 'lucide-react'
...
// Palabras de unidad comunes a depurar
...
export default function IngresoLoteHubModal({ isOpen, onClose, productos = [], categorias = [], onSuccess }) {
  ...
  const iniciarOcrConImagenMejorada = async (imageSrc, file) => {
  ...
  const crearFilaObjeto = (codigo, nombre, cantidad, costo, precio, catSugerida = '', unidad = 'und') => {
              ...
              {tab === 'foto' && (
                <div className="space-y-4 h-full flex flex-col justify-center items-center min-h-[300px]">
                  ...
                  {isProcessingFile ? (
                    ...
                    >
                      <UploadCloud className="text-slate-400" size={56} />
