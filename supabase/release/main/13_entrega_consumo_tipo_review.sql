-- Release 13 (principal): espejo del parche 274 de staging.
-- La rama 'Saldo a favor' de confirmar_entrega_finanzas_idempotente insertaba
-- tipo 'abono' (pago de deuda) en vez de 'consumo_credito' y decrementaba la
-- deuda → neteaba el cargo COD y lo dejaba incobrable (caso #3072).
-- Parche: tipo correcto + saldo_usd desde el bolsillo de favor (replay
-- canónico del ledger). Fuente: dump vivo del principal (13_backup_cuerpos).
-- Rollback: 13_rollback_review.sql (cuerpo previo byte-idéntico).
    ...
    ELSIF lower(v_metodo) = 'saldo a favor' AND v_cliente_id IS NOT NULL THEN
      IF NOT EXISTS (
        ...
        INSERT INTO public.cuentas_por_cobrar (
          ...
          v_cliente_id, p_despacho_id, 'consumo_credito', v_monto, v_saldo,
...
BEGIN
  ...
  WHERE n.nspname = 'public'
    AND p.proname = 'confirmar_entrega_finanzas_idempotente'
    AND position('consumo_credito'', v_monto' in pg_get_functiondef(p.oid)) = 0;
  IF v_bad > 0 THEN
    RAISE EXCEPTION 'POSTFLIGHT 13: funcion sin el tipo consumo_credito';
  END IF;
END $$;
