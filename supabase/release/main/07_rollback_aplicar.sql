-- Rollback 07 generado automáticamente (backup 2026-09-05T06:36:00.647Z)
BEGIN;
UPDATE public.configuracion_negocio SET comision_split_activo = false;
DELETE FROM public.comisiones c WHERE (c.calculo_evidencia->>'split_cliente_ajeno')::boolean IS TRUE;
DROP INDEX IF EXISTS public.ux_comisiones_despacho_vendedor;
CREATE UNIQUE INDEX IF NOT EXISTS ux_comisiones_despachoid_238a ON public.comisiones (despachoid);
