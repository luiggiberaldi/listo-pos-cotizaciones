-- 239: reemplazo atomico de items en edicion profunda
--
-- Este archivo debe ejecutarse dentro de una transaccion controlada.
-- El runner scripts/apply-deep-edit-main.mjs captura backup, ejecuta este SQL,
-- valida la funcion y prueba el flujo antes de hacer COMMIT.
-- No limpia despachos historicos.
  ...
  IF EXISTS (
    ...
    FROM (
      SELECT
        ...
        COALESCE(
          NULLIF(trim(x.origen), ''),
          CASE WHEN x.producto_id IS NULL THEN 'externo' ELSE 'inventario' END
        ) AS origen_key,
    ...
    RAISE EXCEPTION
      'ITEMS_DUPLICADOS: existe mas de una linea con la misma condicion comercial';
  END IF;
END;
$$;
...
BEGIN
  ...
  IF p_nuevos_items IS NULL
    ...
    RAISE EXCEPTION 'ITEMS_INVALIDOS: Debe proporcionar al menos un item';
  END IF;
  ...
  IF NOT FOUND OR COALESCE(v_operador.activo, false) IS NOT TRUE THEN
    RAISE EXCEPTION 'ACCESO_DENEGADO: Operador invalido o inactivo';
  END IF;

  IF v_operador.rol NOT IN (
    'administracion',
    'jefe',
    'desarrollador'
  ) THEN
    RAISE EXCEPTION
      'ACCESO_DENEGADO: Rol no autorizado para editar despachos a profundidad';
  END IF;

  IF p_usuario_rol IS DISTINCT FROM v_operador.rol THEN
    RAISE EXCEPTION
      'ACCESO_DENEGADO: El rol informado no coincide con el operador';
  END IF;
  ...
  IF v_despacho.estado <> 'pendiente' THEN
    RAISE EXCEPTION
      'ESTADO_INVALIDO: Solo se puede editar un despacho pendiente';
  END IF;

  -- Toda validacion ocurre antes del DELETE. Un payload invalido conserva
  -- intacto el snapshot actual.
  PERFORM public.validar_items_despacho_sin_duplicados(p_nuevos_items);

  FOR v_item_json IN
    SELECT *
    FROM jsonb_to_recordset(p_nuevos_items) AS x(
      producto_id uuid,
      codigo_snap text,
      nombre_snap text,
      unidad_snap text,
  ...
  LOOP
    IF v_item_json.nombre_snap IS NULL
       OR trim(v_item_json.nombre_snap) = '' THEN
      RAISE EXCEPTION 'ITEM_INVALIDO: El nombre del producto es obligatorio';
    END IF;

    IF v_item_json.cantidad IS NULL
       OR v_item_json.cantidad <= 0 THEN
      RAISE EXCEPTION 'ITEM_INVALIDO: La cantidad debe ser mayor que cero';
    END IF;

    IF v_item_json.precio_unit_usd IS NULL
       OR v_item_json.precio_unit_usd < 0 THEN
    ...
    IF v_item_json.producto_id IS NULL
       AND v_origen <> 'externo' THEN
      RAISE EXCEPTION
        'ITEM_INVALIDO: Una linea sin producto_id debe ser externa';
    END IF;
    ...
    IF v_item_json.producto_id IS NOT NULL THEN
      ...
      IF NOT FOUND
        ...
        RAISE EXCEPTION
          'PRODUCTO_INVALIDO: Producto no disponible en la cuenta del despacho';
      END IF;
    END IF;
  END LOOP;

  RAISE NOTICE
    'DEEP_EDIT_RPC_BEFORE_REPLACE despacho=% item_count=%',
    p_despacho_id,
    jsonb_array_length(p_nuevos_items);

  -- Los despachos pendientes aun no han descontado stock. El reemplazo es
  -- atomico y solo afecta las lineas del despacho bloqueado.
  ...
  LOOP
    ...
    INSERT INTO public.notas_despacho_items (
      ...
      CASE
        WHEN COALESCE(v_item_json.es_prestamo, false) THEN 0.0000
  ...
  RAISE NOTICE
    'DEEP_EDIT_RPC_AFTER_INSERT despacho=% item_count=% total=%',
    p_despacho_id,
    (SELECT COUNT(*) FROM public.notas_despacho_items WHERE despacho_id = p_despacho_id),
    v_total_nuevo;
...
-- El Worker usa el overload de seis parametros con service_role. Se cierran
-- los overloads antiguos para evitar saltarse la validacion nueva.
REVOKE ALL ON FUNCTION public.editar_despacho_profundidad(uuid, jsonb)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.editar_despacho_profundidad(uuid, jsonb, uuid, text, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.editar_despacho_profundidad(uuid, jsonb, uuid, text, text, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.validar_items_despacho_sin_duplicados(jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.editar_despacho_profundidad(uuid, jsonb, uuid, text, text, text)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.validar_items_despacho_sin_duplicados(jsonb)
  TO service_role;
