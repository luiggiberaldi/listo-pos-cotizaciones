-- 12_saldo_favor_consumo_review.sql — RELEASE 12 (principal)
-- Fix de saldo a favor fantasma (espejo de las migraciones 272/273 de staging):
--   272: la fórmula de v_saldo_favor en las RPCs de entrega/devolución no
--        conocía el tipo 'consumo_credito' (introducido 2026-08-28) y pisaba
--        el valor correcto del trigger tras cada entrega — caso real:
--        Antoplast $112 y ROSA SANCHEZ $4.14.
--   273 (solo entrega): el guard anti-duplicado de la rama 'saldo a favor'
--        solo revisaba tipo='abono' e insertaba un abono repetido cuando el
--        Worker ya registró el consumo en la aprobación.
-- Fuente: cuerpos VIVOS del principal (pg_get_functiondef, 2026-09-09),
-- respaldados en 12_backup_cuerpos/ (base del rollback).
-- Verificación: scripts/verify-diff-release12.mjs (diff LCS exacto por función).
...
-- Postflight: las 4 funciones deben conocer consumo_credito y la entrega
-- debe traer el guard anti-duplicado 273.
DO $$
DECLARE
  fns TEXT[] := ARRAY[
    ...
    'registrar_cobro_diferencia_devolucion'
  ];
  f TEXT;
BEGIN
  FOREACH f IN ARRAY fns LOOP
    IF NOT EXISTS (
      ...
      WHERE n.nspname = 'public' AND p.proname = f
        AND position('consumo_credito' in pg_get_functiondef(p.oid)) > 0
    ) THEN
      RAISE EXCEPTION 'POSTFLIGHT R12 FALLO: % sin consumo_credito', f;
    END IF;
  END LOOP;
  IF EXISTS (
    SELECT 1 FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
    WHERE n.nspname = 'public' AND p.proname = 'confirmar_entrega_finanzas_idempotente'
      AND position('AND c.tipo = ''abono''' in pg_get_functiondef(p.oid)) > 0
      AND position('IN (''abono'', ''consumo_credito''' in pg_get_functiondef(p.oid)) = 0
  ) THEN
    RAISE EXCEPTION 'POSTFLIGHT R12 FALLO: entrega sin guard 273';
  END IF;
  RAISE NOTICE 'POSTFLIGHT R12 OK: 4 funciones parcheadas + guard 273';
END
$$;
