-- 03_comision_split_cliente_ajeno.sql
-- Proyecto principal — Comisión split por cliente ajeno (espejo de la 257 de staging).
--
...
-- autorizada, con backup verificado y rollback aprobado (03_..._rollback.sql).
-- Validado end-to-end en staging con la migración 257 + E2E T1–T9.

BEGIN;

-- SAFETY GATE: elimina este bloque únicamente después de aprobar backup,
-- preflight, E2E de staging en verde y rollback.
DO $$
BEGIN
  RAISE EXCEPTION 'REVIEW_ONLY: 03_comision_split_cliente_ajeno.sql no debe ejecutarse todavía';
END
$$;
