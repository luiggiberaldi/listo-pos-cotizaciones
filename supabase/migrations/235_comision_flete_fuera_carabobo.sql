...
-- Regla para choferes locales:
--   * Destino dentro de Carabobo: no se calcula comisión; corresponde a nómina externa.
--   * Destino fuera de Carabobo: se calcula la comisión configurada sobre el flete.
--   * Transportistas no locales: no generan comisión en este módulo.
--
-- La regla vive en la base de datos para cubrir todos los flujos de creación/edición
-- y no depender de valores enviados por el frontend.
...
-- Normaliza mayúsculas, espacios y acentos sin depender de la extensión unaccent.
...
-- Calcula y congela el tratamiento financiero del flete.
...
BEGIN
  -- El Worker inserta con service_role y el trigger general de tenant puede
  -- ejecutarse después de este trigger. Resolvemos la cuenta desde las FKs
  -- antes de leer configuración para no mezclar tenants ni usar siempre id=1.
  ...
  -- Fijar el tenant aquí también hace que el cálculo sea correcto aunque el
  -- trigger set_cuenta_id_smart todavía no haya corrido.
  IF NEW.cuenta_id IS NULL AND v_cuenta_id IS NOT NULL THEN
    NEW.cuenta_id := v_cuenta_id;
  END IF;

  -- Se prioriza la dirección alternativa; si no existe, se usa el estado del
  -- cliente. El valor queda congelado en el despacho.
...
-- Esto cubre también actualizaciones directas hechas por un administrador; la
-- nómina de Carabobo nunca puede entrar a pagos_transportistas por accidente.
...
-- Un flete ya pagado no puede cambiar de destino, chofer o importe sin reversa.
...
BEGIN
  ...
  THEN
    v_rol := public.get_rol_actual();
    -- El Worker/service_role tiene v_rol NULL y puede ejecutar la reversa
    -- o una operación administrativa previamente validada.
...
-- Reemplaza la liquidación atómica para tomar exclusivamente comisiones
-- generadas fuera de Carabobo. La nómina nunca entra en esta tabla/saldo.
...
-- Estas RPCs solo deben ser invocables por el Worker con service_role. La
-- autorización de usuario/rol y el tenant se validan antes en la API.
...
-- No se hace backfill financiero automático: los despachos históricos no tienen
-- necesariamente un snapshot confiable de destino/configuración. Administración
-- puede revisarlos y recalcularlos explícitamente por período.
