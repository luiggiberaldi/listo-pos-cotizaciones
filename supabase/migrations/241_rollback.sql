-- Rollback de 241 para PRODUCCIÓN (oyfyuszgjwcepjpngclv) — generado desde el estado pre-241 capturado.
-- Ejecutar solo si hay que revertir la migración. Transaccional.
BEGIN;

DROP TABLE IF EXISTS public.operator_sessions CASCADE;

-- 1) Eliminar policies RLS creadas por 241 (dependen de las funciones que se eliminan después)
...
-- 2) Restaurar funciones de sesión al estado pre-241 (capturado)
...
-- 3) Restaurar definiciones de reportes (capturadas pre-241)
CREATE OR REPLACE FUNCTION public.obtener_reporte_ventas_operaciones(p_fecha_inicio date DEFAULT NULL::date, p_fecha_fin date DEFAULT NULL::date, p_vendedor_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(despacho_id uuid, despacho_numero integer, cotizacion_id uuid, fecha timestamp with time zone, estado text, asesor_id uuid, asesor_nombre text, asesor_color text, cliente_nombre text, total_usd numeric, flete_usd numeric, descuento_usd numeric, venta_neta_usd numeric, tasa numeric, total_bs numeric, forma_pago jsonb, referencia_pago text, cliente_tipo_cliente text, cliente_categoria text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.obtener_reporte_ventas_comisiones(p_fecha_inicio timestamp with time zone DEFAULT NULL::timestamp with time zone, p_fecha_fin timestamp with time zone DEFAULT NULL::timestamp with time zone, p_vendedor_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(despacho_id uuid, despacho_numero integer, fecha timestamp with time zone, asesor text, asesor_color text, cliente text, codigo text, descripcion text, pza text, precio numeric, cantidad numeric, total numeric, comision_pct numeric, total_com numeric, tasa numeric, pago text, total_bs numeric, estado text, estado_comision text, despacho_comision_liberada numeric, despacho_comision_total numeric, cliente_tipo_cliente text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- 4) Restaurar grants EXECUTE pre-241 (capturados con aclexplode)
GRANT EXECUTE ON FUNCTION public.obtener_resumen_comisiones(p_cuenta_id uuid, p_vendedor_id uuid, p_estado text, p_fecha_inicio timestamp with time zone, p_fecha_fin timestamp with time zone) TO PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.obtener_resumen_comisiones_v2(p_cuenta_id uuid, p_vendedor_id uuid, p_estado text, p_fecha_inicio timestamp with time zone, p_fecha_fin timestamp with time zone) TO service_role;

-- 5) Restaurar grants de usuarios al estado pre-241 (tabla completa para anon/authenticated)
REVOKE SELECT (id, cuenta_id, activo, nombre, rol, email, telefono, color, codigo, markup_pct, comision_pct, comision_pct_cabilla, es_externo, creado_en, actualizado_en, ultimo_acceso) ON public.usuarios FROM authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON public.usuarios TO anon, authenticated;
