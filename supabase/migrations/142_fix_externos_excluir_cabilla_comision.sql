-- 142_fix_externos_excluir_cabilla_comision.sql
-- Asegurar que los productos externos NUNCA generen comisión de cabilla (2%) 
-- sino que tengan su propia tasa configurable (comision_pct_externos) que por defecto es 3%.

-- ══════════════════════════════════════════════════════════════════════
-- 0. Alterar tabla configuracion_negocio para añadir la tasa de externos
-- ══════════════════════════════════════════════════════════════════════
ALTER TABLE public.configuracion_negocio
  ADD COLUMN IF NOT EXISTS comision_pct_externos NUMERIC(5,2) NOT NULL DEFAULT 3;

COMMENT ON COLUMN public.configuracion_negocio.comision_pct_externos 
  IS 'Porcentaje de comisión para productos externos (por defecto 3%).';
...
BEGIN
  ...
  -- NO generar comisión para los siguientes roles
  IF v_despacho.vendedor_rol IN ('vendedor_sin_comision', 'jefe', 'logistica', 'administracion', 'desarrollador') THEN 
    RETURN NULL; 
  END IF;
  ...
  -- Sumamos la comisión de externos en comision_otros para mantener la compatibilidad de columnas
