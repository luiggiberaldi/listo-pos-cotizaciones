-- 128_fix_rpc_editar_despacho_externos.sql
-- Permite que editar_despacho_profundidad acepte productos externos (sin producto_id) saltándose la validación de stock.
...
BEGIN
  ...
  FOR v_item_json IN SELECT * FROM jsonb_to_recordset(p_nuevos_items) AS x(
    ...
    -- Validar y descontar stock SOLO si es producto de inventario
