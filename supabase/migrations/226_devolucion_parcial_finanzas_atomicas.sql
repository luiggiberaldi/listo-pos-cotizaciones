-- 226: Finanzas atómicas de devolución parcial
--
-- La RPC 225 ya consolida stock, Kardex y documentos de devolución. Esta
-- migración agrega el ajuste financiero y un wrapper que ejecuta ambas partes
-- dentro de la misma transacción HTTP de PostgREST.
...
CREATE OR REPLACE FUNCTION public.registrar_devolucion_parcial_atomica(
  p_despacho_id             UUID,
...
BEGIN
  ...
  v_finanzas := public.ajustar_finanzas_devolucion_atomica(
    p_despacho_id,
    p_total_devuelto_usd,
    p_total_intercambio_usd,
    p_usuario_id,
  ...
  RETURN v_inventario || v_finanzas || jsonb_build_object('transaccion_atomica', TRUE);
END;
$$;
...
REVOKE ALL ON FUNCTION public.registrar_devolucion_parcial_atomica(UUID, JSONB, JSONB, TEXT, UUID, TEXT, TEXT, UUID, NUMERIC, NUMERIC)
  FROM PUBLIC, authenticated;
GRANT EXECUTE ON FUNCTION public.registrar_devolucion_parcial_atomica(UUID, JSONB, JSONB, TEXT, UUID, TEXT, TEXT, UUID, NUMERIC, NUMERIC)
  TO service_role;
