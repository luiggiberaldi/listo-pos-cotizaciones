-- ═══════════════════════════════════════════════════════════════════════════
-- 270 (staging): Comisión solo cuando el COD está pagado (v4, todo-o-nada)
-- ═══════════════════════════════════════════════════════════════════════════
-- Regla de negocio: despacho con porción "Cobro a destino" (monto > 0) y
-- cobro_destino_pagado ≠ true → CERO comisión (incluye pagos mixtos con
-- adelanto: no se paga la parte adelantada hasta conciliar el COD). Al
-- conciliar, la comisión completa se genera sobre el total.
--
--   1) Toggle por cuenta: comision_cod_solo_pagado (default OFF = sin cambio
--      de comportamiento hasta activación explícita).
--   2) Helper comision_238b_cod_pendiente(forma_pago_cliente, forma_pago):
--      detecta COD pendiente en todos los formatos legacy (string, objeto,
--      array). Escanea elementos de nivel superior (el flag
--      cobro_destino_pagado vive en el elemento COD mismo).
--   3) calcularcomisiondespacho_238b: cuerpo vivo de staging + tranca
--      RETURN NULL después de la carga de configuración y antes del split —
--      convención "sin comisión" existente: 0 filas para nadie.
--      El disparador de conciliación (Worker) llama a esta función al
--      marcar cobro_destino_pagado=true → la comisión nace al cobrar.
--   Nota: recalcularcomisiondespacho_238b no cambia (G3 intacto: jamás retira
--   filas pagadas/parciales). Con tranca activa, recalcular sobre despacho
--   COD-impago borra filas 238b no pagadas y el calcular devuelve NULL →
--   queda sin comisión, coherente con la regla.
-- ═══════════════════════════════════════════════════════════════════════════
...
ALTER TABLE public.configuracion_negocio
  ADD COLUMN IF NOT EXISTS comision_cod_solo_pagado boolean NOT NULL DEFAULT false;

-- ── 2) Helper: ¿tiene el despacho COD pendiente de cobro? ────────────────────
CREATE OR REPLACE FUNCTION public.comision_238b_cod_pendiente(
...
BEGIN
  v_source := COALESCE(
    NULLIF(btrim(COALESCE(p_forma_pago_cliente, '')), ''),
    NULLIF(btrim(COALESCE(p_forma_pago, '')), '')
  );
  IF v_source IS NULL THEN RETURN FALSE; END IF;

  -- Formato legacy no-JSON: la marca textual basta para asumir COD presente
  -- (no puede tener flag de pagado en ese formato).
  IF NOT pg_input_is_valid(v_source, 'jsonb') THEN
    RETURN lower(v_source) LIKE '%cobro a destino%';
  END IF;
  ...
  IF jsonb_typeof(v_json) = 'object' THEN
    ...
    END IF;
  END IF;
  IF jsonb_typeof(v_json) <> 'array' THEN RETURN FALSE; END IF;

  -- El elemento "Cobro a destino" vive en el nivel superior del array; su
  -- flag cobro_destino_pagado y su monto están en el propio elemento.
  FOR v_elem IN
    SELECT e.elem FROM jsonb_array_elements(v_json) AS e(elem)
    WHERE jsonb_typeof(e.elem) = 'object'
  LOOP
    v_met := lower(trim(COALESCE(
      v_elem->>'metodo', v_elem->>'metodo_pago', v_elem->>'method', v_elem->>'formaPago', ''
    )));
    IF v_met = 'cobro a destino' THEN
      v_monto := CASE
        WHEN COALESCE(v_elem->>'monto', v_elem->>'amount', v_elem->>'valor', '')
             ~ '^-?[0-9]+([.][0-9]+)?$'
        THEN COALESCE(v_elem->>'monto', v_elem->>'amount', v_elem->>'valor')::numeric
        ELSE 0
      END;
      IF v_monto > 0
         AND NOT COALESCE((v_elem->>'cobro_destino_pagado')::boolean, FALSE) THEN
        RETURN TRUE;
      END IF;
    END IF;
  END LOOP;
...
-- ── 3) calcularcomisiondespacho_238b con tranca COD ──────────────────────────
...
BEGIN
  ...
  -- ── Tranca COD (v4: comisión solo cuando el COD está pagado) ────────────────
  -- Regla todo-o-nada: despacho con porción "Cobro a destino" (monto > 0) y
  -- cobro_destino_pagado ≠ true → CERO comisión (ni dueño ni designado split;
  -- tampoco la parte de adelanto/seña en pagos mixtos). Toggle por cuenta.
  IF COALESCE(v_cfg.comision_cod_solo_pagado, FALSE)
     AND public.comision_238b_cod_pendiente(v_despacho.forma_pago_cliente, v_despacho.forma_pago) THEN
    RETURN NULL;
  END IF;
...
END
$function$;
-- ── Grants + NOTIFY (patrón 269) ─────────────────────────────────────────────
GRANT EXECUTE ON FUNCTION public.calcularcomisiondespacho_238b(uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.comision_238b_cod_pendiente(text, text) TO authenticated, service_role;
