-- 200: comisión CxC manual — eliminar liberación proporcional automática
--
-- MODELO NUEVO (acordado con negocio):
--   * Contado + COD ("Cobro a destino")  -> comisión LIBERADA al aprobar el despacho.
--   * CxC ("Cta por cobrar")              -> comisión RETENIDA; administración la libera a mano.
--   * `totalcomision` NO cambia: solo se redefine el split liberada/retenida.
--       - Antes: retenida = CxC + COD.
--       - Ahora: retenida = SOLO CxC.
--   * Se elimina el trigger que liberaba el remanente al saldar (185/191).
--
-- monto_cxc se calcula desde el JSON `forma_pago` (fuente de verdad), no desde los cargos,
-- para no depender del orden de creación del cargo ni del path directo a 'entregada'.
--
-- El evento 'contado' se fecha con la aprobación real (despachada_en), no con now().

-- ── 1. Permitir el tipo 'manual' en los eventos de liberación ──────────────────
ALTER TABLE public.comision_liberaciones DROP CONSTRAINT IF EXISTS comision_liberaciones_tipo_check;
ALTER TABLE public.comision_liberaciones
  ADD CONSTRAINT comision_liberaciones_tipo_check CHECK (tipo IN ('contado','abono','manual'));

-- ── 2. Eliminar el automatismo de liberación por pago ──────────────────────────
...
DROP FUNCTION IF EXISTS public.trg_liberar_comision_por_pago();

-- ── 3. Redefinir calcularcomisiondespacho con el nuevo split ───────────────────
...
BEGIN
  ...
  -- ── Split: CxC retenido (admin lo libera a mano); contado + COD liberado al aprobar ──
  DECLARE
  ...
  BEGIN
    ...
    -- monto_cxc desde el JSON forma_pago (suma de los métodos 'Cta por cobrar')
    BEGIN
    ...
    EXCEPTION WHEN others THEN
      -- Texto plano legacy: si es exactamente 'Cta por cobrar', todo es crédito
    ...
    IF v_liberada > 0 THEN
      INSERT INTO public.comision_liberaciones (comision_id, despacho_id, vendedor_id, cuenta_id, monto, tipo, creado_en)
      VALUES (v_comisionid, p_despachoid, v_despacho.vendedor_dueno_cliente_id, v_despacho.cuenta_id, v_liberada, 'contado', v_fecha_aprob);
    END IF;
  END;
