-- 252_staging_devolucion_cobro_diferencia.sql
-- Staging — Cobro de la diferencia en devolución parcial (Opción A).
--
-- ADDITIVO: no altera la firma existente de 13 parámetros de
-- registrar_devolucion_parcial_finanzas_atomica_staging. Publica una
-- SOBRECARGA de 14 parámetros con p_pagos_diferencia JSONB DEFAULT NULL.
-- El wrapper nuevo:
...
--
-- Rollback: DROP FUNCTION de la sobrecarga de 14 args y de los dos helpers.
-- La firma de 13 args y los datos quedan intactos.

-- ---------------------------------------------------------------------------
-- Helper de validación (sin efectos, lanza excepción ante payload inválido).
-- Se llama antes de reservar la clave idempotente para fallar rápido.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Helper de registro: inserta un abono por cada pago cobrado y recalcula los
-- saldos del cliente. Debe invocarse DESPUÉS de la RPC de devolución dentro
-- de la misma transacción (el cargo de diferencia ya existe a ese punto).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.registrar_devolucion_parcial_finanzas_atomica_staging(
  UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID, TEXT, TEXT, UUID, NUMERIC, NUMERIC, JSONB, JSONB
);

CREATE OR REPLACE FUNCTION public.registrar_devolucion_parcial_cobro_staging(
  p_cuenta_id                UUID,
  p_despacho_id              UUID,
...
REVOKE ALL ON FUNCTION public.registrar_cobro_diferencia_devolucion(UUID, UUID, UUID, TEXT, NUMERIC, NUMERIC, JSONB)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.registrar_devolucion_parcial_cobro_staging(UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID, TEXT, TEXT, UUID, NUMERIC, NUMERIC, JSONB, JSONB)
  FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.registrar_cobro_diferencia_devolucion(UUID, UUID, UUID, TEXT, NUMERIC, NUMERIC, JSONB)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.registrar_devolucion_parcial_cobro_staging(UUID, UUID, UUID, JSONB, JSONB, TEXT, UUID, TEXT, TEXT, UUID, NUMERIC, NUMERIC, JSONB, JSONB)
  TO service_role;
