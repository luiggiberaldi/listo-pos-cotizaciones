-- 260_staging_comision_split_designado.sql
--
-- CAMBIO DE MODELO (decisión del negocio 2026-09-04, plan docs/plans/2026-09-04-plan-comision-split-designado-sabados-v3.md):
-- El split del sábado ya no es "vendedor ajeno vs dueño del cliente". Ahora:
--   · El rol JEFE designa a UNA persona (vendedor o supervisor) por día
--     → tabla comision_designacion_diaria, UNIQUE (cuenta_id, fecha).
--   · Ese día, si un vendedor/supervisor elegible vende a un cliente AJENO
--     (dueño del cliente ≠ vendedor que vendió), el % general se divide:
--       - comision_split_pct_vendedor (0.5%) → fila para el DESIGNADO del día
--       - comision_split_pct_dueno    (1.5%) → fila para el DUEÑO del cliente
--     El vendedor que vendió NO cobra nada en esa venta.
--   · Venta a cliente PROPIO del vendedor → comisión normal completa (sin división).
--   · Designado = dueño del cliente → una sola fila con su % normal (es dueño).
--   · Sin designación ese día → todo normal (aunque sea cliente ajeno).
--   · Elegibles: vendedor y supervisor activos no externos. Excluidos: vendedor_sin_comision
--     interno sin markup, jefe, admin, administracion, logistica, desarrollador.
--   · Externos no generan split (ni como vendedor que vende ni como dueño).
--   · Los porcentajes del split NUNCA superan el % general vigente (guard en INSERT).
--
-- ADITIVO y re-ejecutable: CREATE TABLE IF NOT EXISTS, CREATE OR REPLACE del MISMO
-- nombre/firma (jamás sobrecarga — lección 42725). No toca datos históricos (forward-only).
-- El guard trigger 238b y el RLS existente no cambian.

BEGIN;

-- ─────────────────────────────────────────────────────────────────────────────
-- 1) Tabla de designación diaria (un solo designado por cuenta y fecha)
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF NOT FOUND OR v_activo IS NOT TRUE
    ...
    RAISE EXCEPTION 'DESIGNADO_INVALIDO: solo vendedor o supervisor activos pueden ser designados';
  END IF;
  RETURN NEW;
END;
$function$;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2) calcularcomisiondespacho_238b — v4 con designado del día
--    Misma firma que la v3 de la 257 (sin overloads — lección 42725).
--    Las bases de cálculo (items, buckets, extras, payment_split, CxC) no
--    cambian; solo la DECISIÓN de split y a quién se le insertan las filas.
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Requisitos: config activa ∧ día configurado ∧ HAY designado válido ese día
  --             ∧ vendedor que vende ≠ dueño del cliente (cliente ajeno)
  --             ∧ vendedor elegible (vendedor/supervisor no externo)
  --             ∧ dueño del cliente no externo.
  -- Cliente propio del vendedor → comisión normal, sin split.
    ...
    IF v_seller_rol IN ('vendedor', 'supervisor')
      ...
      IF v_split_aplica THEN
        ...
        WHERE d.cuenta_id = v_despacho.cuenta_id
          AND d.fecha = v_despacho.creado_en::date
          AND EXISTS (
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 3) Grants (patrón 257/239): solo service_role ejecuta la RPC/trigger
-- ─────────────────────────────────────────────────────────────────────────────
REVOKE ALL ON FUNCTION public.calcularcomisiondespacho_238b(uuid) FROM PUBLIC, anon, authenticated;
-- Tabla de designaciones: el Worker usa service_role; authenticated solo lee.
