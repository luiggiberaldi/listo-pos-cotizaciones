-- 154_fix_comisiones_externos.sql
-- Actualizar calcularcomisiondespacho y obtener_reporte_ventas_comisiones para:
-- 1. Priorizar tasas de comisiones individuales del usuario (usuarios.comision_pct y usuarios.comision_pct_cabilla).
-- 2. Permitir el cálculo de comisión para vendedores externos con rol 'vendedor_sin_comision' pero con markup_pct > 0 (forzando tasas a 0%).
-- 3. Asegurar que los vendedores externos sin comisión pero con markup no sean excluidos de los reportes.

-- ══════════════════════════════════════════════════════════════════════
-- 1. Redefinir calcularcomisiondespacho
-- ══════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- NO generar comisión para roles administrativos, ni para vendedor_sin_comision a menos que sea externo con markup > 0
  IF v_despacho.vendedor_rol IN ('jefe', 'logistica', 'administracion', 'desarrollador') OR 
     (v_despacho.vendedor_rol = 'vendedor_sin_comision' AND COALESCE(v_despacho.vendedor_markup_pct, 0) <= 0) THEN 
    RETURN NULL; 
  END IF;
  ...
  SELECT
    ...
    COALESCE(cn.comision_pct_externos, 3),
    COALESCE(NULLIF(trim(cn.comision_categoria_cabilla), ''), 'Cabilla')
  INTO v_pct_cabilla, v_pct_otros, v_pct_externos, v_cat_cabilla
  ...
  -- Priorizar tasas individuales del vendedor. Si es vendedor_sin_comision, forzar tasas a 0%
  IF v_despacho.vendedor_rol = 'vendedor_sin_comision' THEN
    v_pct_cabilla := 0;
    v_pct_otros := 0;
    v_pct_externos := 0;
  ELSE
    v_pct_cabilla := COALESCE(v_despacho.vendedor_comision_pct_cabilla, v_pct_cabilla);
    v_pct_otros   := COALESCE(v_despacho.vendedor_comision_pct, v_pct_otros);
    v_pct_externos := COALESCE(v_despacho.vendedor_comision_pct, v_pct_externos);
  END IF;
  ...
  IF v_tiene_items_despacho THEN
    SELECT
      COALESCE(SUM(CASE
        -- Si es un producto externo, NUNCA genera comisión de cabilla
        WHEN ndi.origen = 'externo' THEN 0
        WHEN lower(trim(COALESCE(p.categoria,''))) = v_cat_cabilla THEN ndi.total_linea_usd
        ELSE 0
      END), 0),
      COALESCE(SUM(CASE
        -- Si es un servicio de corte, comisión es 0
        WHEN lower(trim(ndi.nombre_snap)) LIKE 'corte%' THEN 0
        -- Si es un producto externo, no entra aquí, tiene su propia suma
        WHEN ndi.origen = 'externo' THEN 0
        -- Si es cabilla de inventario, genera comisión de cabilla (no entra aquí)
      ...
      COALESCE(SUM(CASE
        -- Si es un servicio de corte, comisión es 0
        WHEN lower(trim(ndi.nombre_snap)) LIKE 'corte%' THEN 0
        -- Si es un producto externo general, genera comisión de externos
...
-- ══════════════════════════════════════════════════════════════════════
-- 2. Redefinir obtener_reporte_ventas_comisiones (RPC del PDF)
-- ══════════════════════════════════════════════════════════════════════
