-- 140_fix_roles_comision.sql
-- Fix: Actualizar exclusión de roles que no generan comisiones.
-- Los roles que NO generan comisiones son: vendedor_sin_comision, jefe, logistica, administracion y desarrollador.
...
-- También arreglamos la consulta del reporte de comisiones en PDF
...
BEGIN
  ...
  items_con_comision AS (
    ...
    -- Excluir ítems de roles que no generan comisión del reporte detallado
    AND COALESCE(cl.vendedor_id, nd.vendedor_id) NOT IN (
      SELECT id FROM public.usuarios WHERE rol IN ('vendedor_sin_comision', 'jefe', 'logistica', 'administracion', 'desarrollador')
    )
  )
  SELECT
