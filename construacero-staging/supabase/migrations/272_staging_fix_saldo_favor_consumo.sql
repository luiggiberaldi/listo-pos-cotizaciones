-- 272_staging_fix_saldo_favor_consumo.sql
-- Fix: la formula de v_saldo_favor en las RPCs de entrega/devolucion no
-- conocia el tipo 'consumo_credito' (introducido 2026-08-28, commit db58bd3).
-- Tras cada entrega, la RPC pisaba el valor correcto del trigger con la
-- formula vieja -> saldo a favor fantasma (caso real en principal: Antoplast
-- $112, ROSA SANCHEZ $4.14). Leccion 262-vs-257: un invariante con N
-- escritores diverge si no se auditan juntos.
-- Fuente: cuerpos VIVOS de staging (pg_get_functiondef, 2026-09-09).
-- Cambio por funcion: UNA linea insertada en el CASE de v_saldo_favor:
--   WHEN c.tipo = 'consumo_credito' THEN -c.monto_usd
-- Rollback: supabase/release/staging/272_backup_cuerpos/ (cuerpos originales).
...
-- Postflight: cada funcion debe conocer consumo_credito
DO $$
DECLARE
  fns TEXT[] := ARRAY[
...
BEGIN
  FOREACH f IN ARRAY fns LOOP
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION '272_POSTFLIGHT_FAIL: % sin consumo_credito', f;
    END IF;
  END LOOP;
END $$;
