...
BEGIN
  ...
  IF cardinality(v_despacho_ids) > 0 THEN
    ...
    -- La corrección de fecha mantiene un historial con FK restrictiva.
    -- El bloque condicional conserva compatibilidad con instalaciones staging
    -- anteriores a la migración 241.
    IF to_regclass('public.despacho_fecha_entrega_cambios') IS NOT NULL THEN
      DELETE FROM public.despacho_fecha_entrega_cambios
      WHERE despacho_id = ANY(v_despacho_ids)
        AND cuenta_id = v_cuenta_id;
    END IF;
