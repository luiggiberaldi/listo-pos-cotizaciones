-- 273_staging_entrega_sin_duplicar_saldo_favor.sql
-- Fix: el guard anti-duplicado de la rama "saldo a favor" en las RPCs de
-- entrega solo revisaba tipo='abono'. El Worker (paso 5.b de la aprobacion)
-- ya registra el consumo como tipo='consumo_credito', asi que la entrega
-- insertaba ADEMAS un abono duplicado -> doble descuento del saldo a favor
-- en el ledger del cliente (la columna quedaba bien por el trigger, pero el
-- historial y el replay divergen; la fórmula de columna con abono duplicado
-- da 0 por casualidad aritmetica: consumo -40 y abono -40 se cancelan con el
-- credito +40 solo si el monto coincide exactamente).
-- Expuesto por: construacero-staging/scripts/test-saldo-favor-272.mjs (T2).
-- Fuente: cuerpos VIVOS de staging post-272 (pg_get_functiondef, 2026-09-09).
-- Cambio por funcion: UNA linea modificada en el guard NOT EXISTS:
--   AND c.tipo = 'abono'  ->  AND c.tipo IN ('abono', 'consumo_credito')
-- Rollback: tmp/273/*.live.sql (cuerpos originales post-272).
  ...
  LOOP
    ...
    ELSIF lower(v_metodo) = 'saldo a favor' AND v_cliente_id IS NOT NULL THEN
      IF NOT EXISTS (
        ...
        FROM public.cuentas_por_cobrar c
        WHERE c.despacho_id = p_despacho_id
          AND c.tipo IN ('abono', 'consumo_credito')
  ...
  IF v_cliente_id IS NOT NULL THEN
    SELECT
      ...
      COALESCE(SUM(CASE
        ...
        WHEN c.tipo = 'consumo_credito' THEN -c.monto_usd
        ELSE 0 END), 0)
...
CREATE OR REPLACE FUNCTION public.confirmar_entrega_finanzas_atomica_staging(p_cuenta_id uuid, p_despacho_id uuid, p_idempotency_key uuid, p_usuario_id uuid, p_usuario_nombre text, p_usuario_color text DEFAULT NULL::text, p_tasa_snapshot numeric DEFAULT NULL::numeric, p_permitir_negativo boolean DEFAULT NULL::boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- Postflight: cada funcion debe considerar consumo_credito en el guard
DO $$
DECLARE
  fns TEXT[] := ARRAY[
  ...
  FOREACH f IN ARRAY fns LOOP
    IF NOT EXISTS (
      ...
      WHERE n.nspname = 'public'
        AND p.proname = f
        AND pg_get_functiondef(p.oid) LIKE '%IN (''abono'', ''consumo_credito'')%'
    ) THEN
      RAISE EXCEPTION 'POSTFLIGHT 273 FALLO: % sin guard consumo_credito', f;
    END IF;
  END LOOP;
  RAISE NOTICE 'POSTFLIGHT 273 OK: 2 funciones con guard anti-duplicado';
END
$$;
