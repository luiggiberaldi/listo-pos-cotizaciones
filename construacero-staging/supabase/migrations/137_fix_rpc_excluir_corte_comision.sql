-- 137_fix_rpc_excluir_corte_comision.sql
-- Fix: obtener_reporte_ventas_comisiones ahora excluye cortes externos de la comisión,
-- igualando el comportamiento de calcularcomisiondespacho y eliminándolos por completo del reporte.
...
BEGIN
  ...
  items_con_descuento AS (
    -- Ítems desde notas_despacho_items (con flag de corte externo)
    SELECT
      ndi.id AS item_id,
      nd.cotizacion_id,
      ...
      -- Excluir cortes externos (mismo criterio que calcularcomisiondespacho)
      CASE
        WHEN ndi.origen = 'externo' AND lower(ndi.nombre_snap) LIKE '%corte%' THEN TRUE
        ELSE FALSE
      END AS es_corte_externo
    ...
    -- Ítems desde cotizacion_items (fallback cuando no hay items de despacho)
    SELECT
      ci.id AS item_id,
      ci.cotizacion_id,
      nd.id AS despacho_id_ref,
      ...
      CASE
        WHEN ci.origen = 'externo' AND lower(ci.nombre_snap) LIKE '%corte%' THEN TRUE
        ELSE FALSE
      END AS es_corte_externo
    FROM despachos_filtrados nd
  ...
  SELECT
    ...
    -- % comisión: corte externo → 0%, cabilla → pct_cabilla, otros → pct_otros
    (CASE
      WHEN i.es_corte_externo THEN 0
    ...
    -- Monto comisión: corte externo → $0
  ...
  WHERE NOT i.es_corte_externo  -- Excluir los cortes por completo para que no aparezcan en la tabla del PDF
  ORDER BY COALESCE(nd.entregada_en, nd.creado_en) DESC, i.nombre_snap ASC;
END;
$$;

GRANT EXECUTE ON FUNCTION public.obtener_reporte_ventas_comisiones(TIMESTAMPTZ, TIMESTAMPTZ, UUID) TO authenticated, service_role;
