...
export async function validateOperator(request, env, { requireSupervisor = false } = {}) {
  ...
  if (user.operator_id === SUPER_ADMIN_UUID) {
    // El desarrollador virtual no existe en public.usuarios, pero sigue
    // operando dentro de la cuenta autenticada para que los endpoints
    // multi-tenant (transportistas, CxC e inventario) puedan filtrar por ella.
    // Las RPC financieras estrictas necesitan además un actor FK real para
    // `registrado_por`; se resuelve aquí sin convertir la identidad virtual en
    // una excepción dentro de SQL.
    const headers = supaServiceHeaders(env);
    let actorId = null;
    try {
      const actorRes = await fetch(
        `${env.SUPABASE_URL}/rest/v1/usuarios?cuenta_id=eq.${user.id}&activo=eq.true&select=id,rol&order=nombre.asc,id.asc&limit=100`,
        { headers }
      );
      if (actorRes.ok) {
        const rows = await actorRes.json();
        const priority = ['desarrollador', 'administracion', 'supervisor', 'jefe', 'logistica', 'vendedor', 'vendedor_sin_comision'];
        rows.sort((a, b) => {
          const roleDelta = priority.indexOf(a?.rol) - priority.indexOf(b?.rol);
          return (roleDelta || 0) || String(a?.id || '').localeCompare(String(b?.id || ''));
        });
        actorId = rows[0]?.id || null;
      }
    } catch (error) {
      console.warn('[AUTH] No se pudo resolver actor FK para desarrollador virtual:', error?.message);
    }
    const operador = {
      id: SUPER_ADMIN_UUID,
      actor_id: actorId,
    ...
    return { user, operador, headers, ip };
  }
