...
export async function handleSwitchOperator(request, env) {
  // La clave service_role solo vive en el Worker local/deploy; nunca en Vite.
  // Devolver un diagnóstico explícito evita convertir una mala configuración
  // local en un 500 opaco durante el login.
  if (!env.SUPABASE_URL || !env.SUPABASE_SERVICE_KEY) {
    console.error('[SWITCH-OP] Falta SUPABASE_URL o SUPABASE_SERVICE_KEY en el Worker')
    return jsonError('Worker no configurado: falta la clave administrativa de Supabase', 503, request)
  }
