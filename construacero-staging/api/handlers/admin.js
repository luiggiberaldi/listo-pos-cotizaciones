...
import { verifyAuth, verifySupervisor, verifyPrivileged, SUPER_ADMIN_UUID, validateOperator, invalidateOperatorCache } from '../lib/auth.js'
...
export async function handleRestore(request, env) {
  ...
  // Un restore inserta números explícitos y no avanza las secuencias usadas por
  // los RPCs. Repararlas al final evita colisiones en la siguiente operación.
  let secuencias = null;
  const sequenceRes = await fetch(`${env.SUPABASE_URL}/rest/v1/rpc/sync_sequences_after_restore`, {
    method: 'POST',
    headers: h,
    body: '{}',
  });
  if (sequenceRes.ok) {
    secuencias = await sequenceRes.json().catch(() => null);
  } else {
    const errText = await sequenceRes.text().catch(() => 'unknown');
    errores.push(`secuencias: ${errText}`);
  }

  return json({
    ok: errores.length === 0,
    resumen,
    secuencias: secuencias || undefined,
...
export async function handleSaveConfig(request, env) {
  ...
  // G8 (plan split cliente ajeno): validación server-side de los campos nuevos
...
export async function handleResetOperacional(request, env) {
  ...
  try {
    ...
    if (rpcRes.ok) {
      const _rpcData = await rpcRes.json().catch(() => ({}));
    ...
    // El reset operacional conserva productos y su Kardex. Borrar solo los
    // movimientos dejaría stock_actual sin una historia reconciliable.
    logStep('Kardex conservado junto con el inventario.');
...
export async function handleTesterClearAll(request, env) {
  ...
  try {
    ...
    // CLEAR ALL conserva el catálogo y el Kardex; solo elimina datos
    // operacionales. Así no crea una divergencia stock_actual vs Kardex.
    logStep('Kardex conservado junto con el inventario.');
...
async function supaBatch(env, table, rows, chunkSize = 500, _logFn = null) {
...
async function supaDelete(env, table, userId, _logFn = null) {
...
export async function handleCrearTransportista(request, env) {
  ...
  const { nombre, rif, telefono, zona_cobertura, tarifa_base, notas, color, vehiculo, placa_chuto, placa_batea, capacidad, color_batea,
          es_local, tipo_relacion, empleado_id, fecha_inicio_relacion, fecha_fin_relacion, emite_comprobante } = body;
  ...
  const tiposRelacion = ['empleado', 'contratista', 'proveedor', 'propio'];
  const tipoRelacion = tipo_relacion || 'proveedor';
  if (!tiposRelacion.includes(tipoRelacion)) return jsonError('tipo_relacion inválido', 400, request);
  if (empleado_id && !isValidUuid(empleado_id)) return jsonError('empleado_id inválido', 400, request);
...
export async function handleActualizarTransportista(request, env) {
  ...
  if (updateData.tipo_relacion && !['empleado', 'contratista', 'proveedor', 'propio'].includes(updateData.tipo_relacion)) {
    return jsonError('tipo_relacion inválido', 400, request);
  }
  if (updateData.empleado_id && !isValidUuid(updateData.empleado_id)) return jsonError('empleado_id inválido', 400, request);
...
export async function handleDesactivarTransportista(request, env) {
  ...
  // Solo supervisores, jefes, desarrolladores y administración pueden desactivar
  if (!['supervisor', 'jefe', 'desarrollador', 'administracion'].includes(operador.rol)) {
