...
import { validateOperator, SUPER_ADMIN_UUID } from '../lib/auth.js'
import { registrarAuditoria } from '../lib/audit.js'
import { recalcularSaldoPendienteCliente } from '../lib/cxcUtils.js'
import { calcularDescuentoValido } from '../lib/descuentoUtils.js'
...
const ROLES_CAMBIO_FECHA_ENTREGA = new Set(['administracion', 'jefe', 'logistica', 'desarrollador'])
const LEGACY_DISABLED = false

async function replayStagingOperation({ env, headers, cuentaId, idempotencyKey, nuevoEstado, request }) {
  if (!cuentaId || !isValidUuid(idempotencyKey)) return null

  const expectedType = nuevoEstado === 'entregada'
    ? 'dispatch_delivery_financial'
    : ['pendiente', 'despachada', 'anulada'].includes(nuevoEstado)
      ? 'revertir_entrega_finanzas'
      : null
  if (!expectedType) return null

  try {
    const response = await fetch(
      `${env.SUPABASE_URL}/rest/v1/inventario_operaciones_staging?cuenta_id=eq.${cuentaId}&idempotency_key=eq.${idempotencyKey}&select=operacion_tipo,resultado&limit=1`,
      { headers },
    )
    ...
    if (operation.operacion_tipo !== expectedType) {
      return jsonError('Idempotency-Key ya fue utilizada para otra operación.', 409, request)
    }

    const cached = operation.resultado
    if (!cached || cached.ok !== true) {
      return jsonError('La operación idempotente quedó incompleta; no se repetirá la mutación.', 409, request)
    }
    if (cached.nuevo_estado && cached.nuevo_estado !== nuevoEstado) {
      return jsonError('Idempotency-Key reutilizada con un estado diferente.', 409, request)
    }

    return json({
      ...cached,
      ok: true,
      nuevoEstado: cached.nuevo_estado || nuevoEstado,
      idempotencyKey,
      idempotent: true,
    }, 200, request)
  } catch (error) {
    console.warn('[DESPACHOS] No se pudo consultar replay de operación staging:', error?.message)
    return null
  }
}

function errorCambioFechaEntrega(detail, request) {
  const text = String(detail || '')
  if (text.includes('ROL_NO_PERMITIDO') || text.includes('ACTOR_NO_VALIDO')) {
    return jsonError('No tienes permiso para corregir la fecha efectiva de entrega.', 403, request)
  }
  if (text.includes('DESPACHO_NO_ENCONTRADO')) {
    return jsonError('Despacho no encontrado en tu cuenta.', 404, request)
  }
  if (text.includes('COMISION_PAGADA')) {
    return jsonError('La comisión ya tiene un pago registrado; esta corrección requiere un proceso financiero separado.', 409, request)
  }
  if (text.includes('IDEMPOTENCY')) {
    return jsonError('La clave de idempotencia no es válida.', 400, request)
  }
  if (text.includes('ESTADO_INVALIDO') || text.includes('SIN_CAMBIO')) {
    return jsonError('Solo se puede corregir un despacho entregado y con una fecha diferente.', 409, request)
  }
  if (text.includes('FECHA_ENTREGA_')) {
    return jsonError('La fecha no cumple la cronología del despacho.', 422, request)
  }
  return jsonError('No se pudo corregir la fecha efectiva de entrega.', 500, request)
}
...
async function reconciliarDescuentoComisionRemota(despacho, despachoId, headers, env) {
  const descuentosRes = await fetch(
    `${env.SUPABASE_URL}/rest/v1/despacho_descuentos?despacho_id=eq.${despachoId}&select=cotizacion_item_id,monto_usd`,
    { headers }
  )
  if (!descuentosRes.ok) throw new Error(`No se pudieron leer los descuentos: ${await descuentosRes.text()}`)
  const descuentos = await descuentosRes.json()
  if (!Array.isArray(descuentos) || descuentos.length === 0) return null

  const itemIds = [...new Set(descuentos.map(row => row.cotizacion_item_id).filter(Boolean))]
  if (itemIds.length === 0) return null
  const itemsRes = await fetch(
    `${env.SUPABASE_URL}/rest/v1/cotizacion_items?id=in.(${itemIds.join(',')})&select=id,producto_id,total_linea_usd,origen,nombre_snap`,
    { headers }
  )
  if (!itemsRes.ok) throw new Error(`No se pudieron leer los ítems descontados: ${await itemsRes.text()}`)
  const items = await itemsRes.json()
  const productIds = [...new Set((items || []).map(row => row.producto_id).filter(Boolean))]
  const productsRes = productIds.length > 0
    ? await fetch(`${env.SUPABASE_URL}/rest/v1/productos?id=in.(${productIds.join(',')})&select=id,categoria`, { headers })
    : null
  if (productsRes && !productsRes.ok) throw new Error(`No se pudieron leer las categorías: ${await productsRes.text()}`)
  const products = productsRes ? await productsRes.json() : []

  const configRes = await fetch(
    `${env.SUPABASE_URL}/rest/v1/configuracion_negocio?cuenta_id=eq.${despacho.cuenta_id}&select=comision_pct_cabilla,comision_pct_otros,comision_pct_externos,comision_categoria_cabilla,_comision_extras,_comision_ext_extras,comision_ext_pct_cabilla,comision_ext_pct_otros,comision_ext_pct_externos&limit=1`,
    { headers }
  )
  if (!configRes.ok) throw new Error(`No se pudo leer la configuración de comisiones: ${await configRes.text()}`)
  const [config] = await configRes.json()
  if (!config) return null

  const commissionRes = await fetch(
    `${env.SUPABASE_URL}/rest/v1/comisiones?despachoid=eq.${despachoId}&select=id,totalcomision,comisioncabilla,comisionotros,comision_liberada,comision_retenida,montopagado,estado`,
    { headers }
  )
  if (!commissionRes.ok) throw new Error(`No se pudo leer la comisión: ${await commissionRes.text()}`)
  const [commission] = await commissionRes.json()
  if (!commission) return null

  const calculation = calcularComisionConDescuentos({
    discounts: descuentos,
    items: items || [],
    products: products || [],
    config,
  })
  const patch = reconciliarComisionConDescuento(commission, calculation)
  if (!patch) return null

  const patchRes = await fetch(`${env.SUPABASE_URL}/rest/v1/comisiones?id=eq.${commission.id}`, {
    method: 'PATCH',
    headers: { ...headers, Prefer: 'return=representation' },
    body: JSON.stringify(patch),
  })
  if (!patchRes.ok) throw new Error(`No se pudo ajustar la comisión al descuento: ${await patchRes.text()}`)
  const [updated] = await patchRes.json()
  if (!updated) throw new Error('La comisión no fue actualizada después de aplicar el descuento')
  console.log('[COMISION] Ajuste neto por descuento aplicado:', JSON.stringify({
    despachoId,
    totalAnterior: commission.totalcomision,
    totalNuevo: updated.totalcomision,
  }))
  return updated
}
  ...
  try {
    ...
    if (formaPago) {
      try {
        ...
        // La forma de pago puede llegar como texto legacy; se conserva el valor por defecto.
      }
    }
    ...
    // La regla financiera del flete se aplica en el trigger SQL de la migración
    // 236. No aceptar ni calcular snapshots enviados por el cliente/API.
...
export async function handleCambiarFechaEntregaDespacho(request, env) {
  const v = await validateOperator(request, env)
  if (v.error) return v.error
  const { operador, headers, ip } = v

  if (!ROLES_CAMBIO_FECHA_ENTREGA.has(operador.rol)) {
    return jsonError('No tienes permiso para corregir la fecha efectiva de entrega.', 403, request)
  }
  if (!isValidUuid(operador.cuenta_id)) {
    return jsonError('Cuenta del operador no disponible.', 403, request)
  }

  let body
  try { body = await request.json() } catch { return jsonError('Body inválido', 400, request) }

  const despachoId = body?.despachoId
  const rawFecha = typeof body?.nuevaFechaEntrega === 'string' ? body.nuevaFechaEntrega.trim() : ''
  ...
  if (!tieneZonaHoraria(rawFecha)) {
    return jsonError('nuevaFechaEntrega debe incluir zona horaria.', 400, request)
  }
  const parsedFecha = new Date(rawFecha)
  if (!Number.isFinite(parsedFecha.getTime())) return jsonError('nuevaFechaEntrega inválida', 400, request)
  if (motivo.length < 5) return jsonError('El motivo debe tener al menos 5 caracteres.', 400, request)
  if (motivo.length > 1000) return jsonError('El motivo no puede superar 1000 caracteres.', 400, request)
  if (idempotencyKey.length > 128) return jsonError('Idempotency-Key inválida.', 400, request)

  const rpcRes = await fetch(`${env.SUPABASE_URL}/rest/v1/rpc/cambiar_fecha_entrega_despacho`, {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      p_cuenta_id: operador.cuenta_id,
      p_actor_id: operador.id,
      p_despacho_id: despachoId,
      p_nueva_fecha: parsedFecha.toISOString(),
      p_motivo: motivo,
      p_ip_origen: ip || request.headers.get('CF-Connecting-IP') || null,
      p_idempotency_key: idempotencyKey || null,
    }),
  })

  const text = await rpcRes.text().catch(() => '')
  if (!rpcRes.ok) return errorCambioFechaEntrega(text, request)

  let result
  try { result = text ? JSON.parse(text) : null } catch { result = null }
  const payload = Array.isArray(result) ? result[0] : result
  if (!payload?.ok) return jsonError('La corrección no devolvió confirmación.', 500, request)
...
export async function handleEditarPagoDespacho(request, env) {
  ...
  const camposFlete = transportistaId !== undefined
    || fleteUsd !== undefined
    || direccionEnvioEstado !== undefined
    || clienteId !== undefined
  if (despacho.flete_pagado && camposFlete) {
    return jsonError('El flete ya fue liquidado. Revierta el pago antes de modificar chofer, flete o destino.', 400, request)
  }
  ...
  if (formaPago) {
    try {
      ...
      if (Array.isArray(fps)) {
        ...
        // Operación auxiliar best-effort; no debe bloquear el flujo principal.
      }
  }
  ...
  if (fleteUsd !== undefined || corteUsd !== undefined) {
    const nuevoFlete = fleteUsd !== undefined ? Number(fleteUsd) : Number(despacho.flete_usd || 0);
    const nuevoCorte = corteUsd !== undefined ? Number(corteUsd) : Number(despacho.corte_usd || 0);
    ...
    // El trigger SQL recalcula el snapshot del chofer usando el destino y la
    // configuración vigente; la API no puede imponer un neto propio.
...
export async function handleActualizarEstadoDespacho(request, env) {
  ...
  const replay = await replayStagingOperation({
    env,
    headers,
    cuentaId: operador.cuenta_id,
    idempotencyKey,
  ...
  try {
    ...
    if (esReversionEntregaAtomica) {
      ...
      if (!reversaAtomicaRes.ok) {
        const reversaError = await reversaAtomicaRes.text();
    ...
    // insertaba Kardex en requests separados.
    if (LEGACY_DISABLED && esReversionEntregaAtomica) {
    ...
    // 3b. Entrega: stock, Kardex, estado, CxC y comisión en una transacción.
    if (nuevoEstado === 'entregada') {
      const entregaAtomicaRes = await fetch(`${env.SUPABASE_URL}/rest/v1/rpc/confirmar_entrega_finanzas_atomica_staging`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          p_cuenta_id: operador.cuenta_id,
          ...
          p_usuario_nombre: operador.nombre,
          p_usuario_color: operador.color || null,
          p_tasa_snapshot: tasaBcv && Number(tasaBcv) > 0 ? Number(tasaBcv) : null,
          p_permitir_negativo: null,
        }),
      });
      const entregaText = await entregaAtomicaRes.text();
      let entregaResultado = null;
      try { entregaResultado = entregaText ? JSON.parse(entregaText) : null; } catch { entregaResultado = null; }
      if (!entregaAtomicaRes.ok || !entregaResultado?.ok) {
        const entregaError = entregaResultado?.message || entregaText || `HTTP ${entregaAtomicaRes.status}`;
        console.error('[DESPACHOS] Error en entrega financiera atómica:', entregaError);
        return jsonError(`No se pudo confirmar la entrega: ${entregaError}`, 400, request);
      }
      entregaFinanzasAtomica = entregaResultado;
      if (entregaResultado.idempotent === true) {
        return json({
          ...entregaResultado,
          ok: true,
          nuevoEstado: entregaResultado.nuevo_estado || 'entregada',
          idempotencyKey,
          idempotent: true,
        }, 200, request);
      }
    ...
    // Legacy desactivado: la entrega atómica anterior evita PATCH de stock y
    // POST de Kardex separados. Se conserva temporalmente como referencia para
    // facilitar rollback/revisión del comportamiento histórico.
    if (LEGACY_DISABLED && nuevoEstado === 'entregada') {
    ...
    if (nuevoEstado === 'pendiente' && (desp.estado === 'despachada' || desp.estado === 'entregada')) {
      if (desp.flete_pagado) return jsonError('Revierte primero la liquidación del transportista antes de devolver el despacho', 400, request);
      updateData.motivo_devolucion = body.motivo_devolucion;
      updateData.entregada_en = null;
      // S1-T2: resetear flag de liquidación al chofer para que el despacho re-entre al pool de deudas
      updateData.flete_pagado = false;
    }
    if (nuevoEstado === 'anulada' && (desp.estado === 'despachada' || desp.estado === 'entregada')) {
      if (desp.flete_pagado) return jsonError('Revierte primero la liquidación del transportista antes de anular el despacho', 400, request);
      updateData.motivo_anulacion = body.motivo_anulacion;
      // S1-T2: resetear flag de liquidación al chofer en anulaciones también
      updateData.flete_pagado = false;
    }
    ...
    // 4b. La RPC financiera ya revierte CxC dentro de la misma transacción.
    // Mantener este bloque solo para transiciones no cubiertas por esa RPC.
    if (!esReversionEntregaAtomica && ['pendiente', 'anulada'].includes(nuevoEstado) && (desp.estado === 'despachada' || desp.estado === 'entregada')) {
      try {
        const clienteCxCId = desp.cliente_factura_id || desp.cliente_id;
        if (clienteCxCId) {
    ...
    // 4c. La RPC financiera ya elimina la comisión dentro de la misma transacción.
    if (!esReversionEntregaAtomica && ['despachada', 'entregada'].includes(desp.estado) && ['pendiente', 'anulada'].includes(nuevoEstado)) {
      try {
        await fetch(`${env.SUPABASE_URL}/rest/v1/comisiones?despachoid=eq.${despachoId}`, {
    ...
    if (nuevoEstado === 'despachada') {
      try {
        ...
        const descuentoDespacho = Number(desp.descuento_total_usd || 0)
        let fps = [];
        try {
          ...
          if (Array.isArray(fps)) {
            ...
            if (cxc && Number(cxc.monto) > 0) {
              itemsCargos.push({
                monto: Math.max(0, Number(cxc.monto) - descuentoDespacho),
          ...
          if (fpRaw === 'Cta por cobrar' && Number(desp.total_usd) > 0) {
            itemsCargos.push({
              monto: Math.max(0, Number(desp.total_usd) - descuentoDespacho),
              dias: null,
              metodo: 'cxc',
              desc: ' (Crédito)'
            });
        ...
        if (clienteCxCId) {
              ...
              for (const item of itemsCargos) {
                ...
                const cargoRes = await fetch(`${env.SUPABASE_URL}/rest/v1/cuentas_por_cobrar`, {
                  ...
                  body: JSON.stringify({
                    ...
                    // cuentas_por_cobrar no tiene fecha_vencimiento; esa columna pertenece a CxP.
                    metodo_pago: item.metodo
                  }),
                });
                if (!cargoRes.ok) {
                  const cargoError = await cargoRes.text();
                  throw new Error(`Error al crear cargo CxC: ${cargoError}`);
                }
              }
            }
          }
          ...
          if (abonoSaldoFavor) {
            ...
            if (!Array.isArray(abonoExist) || abonoExist.length === 0) {
              ...
              const [clienteSaldo] = await saldoRes.json();                  const saldoFavorActual = Number(clienteSaldo?.saldo_a_favor || 0);
    ...
    if (!entregaFinanzasAtomica && !esReversionEntregaAtomica && (nuevoEstado === 'despachada' || nuevoEstado === 'entregada')) {
      try {
        ...
        // El desarrollador virtual es el operador de prueba de staging. No es
        // un vendedor administrativo exento: el Tester necesita validar el
        // ciclo completo de comisión con el UUID sintético 0000... que la
        // migración 073 registra en public.usuarios.
        const vendedorVirtual = vendedorIdComision === SUPER_ADMIN_UUID || desp.vendedor_id === SUPER_ADMIN_UUID;
        const rolExento = ['jefe', 'logistica', 'administracion', 'desarrollador'].includes(vendRol?.rol);
        if (!esDonacion && (vendedorVirtual || (!rolExento && (vendRol?.rol !== 'vendedor_sin_comision' || parseFloat(vendRol?.markup_pct || 0) > 0)))) {
          const comRes = await fetch(`${env.SUPABASE_URL}/rest/v1/rpc/calcularcomisiondespacho`, {
            method: 'POST', headers,
            body: JSON.stringify({ p_despachoid: despachoId }),
          });
          const comBody = await comRes.text();
          if (!comRes.ok) {
            console.error('[COMISION] Error al calcular:', comBody);
            if (nuevoEstado === 'entregada') {
              throw new Error(`No se pudo crear la comisión: ${comBody || `HTTP ${comRes.status}`}`);
            }
          } else {
            console.log('[COMISION] RPC calcularcomisiondespacho:', comBody || 'sin retorno');
            if (nuevoEstado === 'entregada') {
              const verifyComRes = await fetch(
                `${env.SUPABASE_URL}/rest/v1/comisiones?despachoid=eq.${despachoId}&select=id`,
                { headers }
              );
              const verifyComRows = verifyComRes.ok ? await verifyComRes.json() : [];
              if (!Array.isArray(verifyComRows) || verifyComRows.length === 0) {
                throw new Error('La entrega terminó sin crear la comisión del despacho');
              }
              await reconciliarDescuentoComisionRemota(desp, despachoId, headers, env);
            }
          }
        } else {
        ...
        if (nuevoEstado === 'entregada') throw comEx;
      }
    }
  ...
  try {
      ...
      if (cliToCheck) {
        ...
        if (cliRes.ok) {
          ...
          if (Array.isArray(cliData) && cliData.length > 0) {
            // La consulta se conserva para compatibilidad con el flujo legacy.
          }
        }
      }
    }
    ...
    if (pagos) {
      try {
        ...
        // Operación auxiliar best-effort; no debe ocultar ni revertir el flujo principal.
      }
    }
...
export async function handleGuardarDescuentos(request, env) {
  ...
  try {
    // 1. Obtener despacho
    const dRes = await fetch(`${env.SUPABASE_URL}/rest/v1/notas_despacho?id=eq.${despachoId}&select=id,estado,cotizacion_id,total_usd,descuento_total_usd`, { headers });
    const [desp] = await dRes.json();
    if (!desp) return jsonError('Despacho no encontrado', 404, request);
    ...
    // 2. Los descuentos se relacionan con cotizacion_items (migración 068),
    // y el frontend envía cotizacionItemId. No usar el id del snapshot
    // notas_despacho_items porque son identificadores distintos.
    const itemsRes = await fetch(
    ...
    if (!itemsRes.ok) return jsonError('No se pudieron cargar los artículos de la cotización', 500, request);
    const items = await itemsRes.json();
    ...
    for (const d of descuentos) {
      const itemId = d.cotizacionItemId || d.cotizacion_item_id;
      if (!itemId || !isValidUuid(itemId)) continue;

      const item = itemMap[itemId];
      if (!item) continue;

      const descuento = calcularDescuentoValido(d.tipo, d.valor, item.total_linea_usd, item.cantidad);
      if (!descuento) continue;
      ...
      descuentosValidos.push({
        ...
        monto_usd: descuento.montoUsd,
        aplicado_por: user.operator_id,
      });
    }

    if (descuentos.length > 0 && descuentosValidos.length === 0) {
      return jsonError('No se encontraron descuentos válidos para los artículos indicados', 400, request);
    }

    // 4. Eliminar descuentos existentes del despacho
    const deleteRes = await fetch(`${env.SUPABASE_URL}/rest/v1/despacho_descuentos?despacho_id=eq.${despachoId}`, {
      method: 'DELETE', headers,
    });
    if (!deleteRes.ok) return jsonError('Error al reemplazar descuentos existentes', 500, request);
    ...
    const totalRes = await fetch(`${env.SUPABASE_URL}/rest/v1/notas_despacho?id=eq.${despachoId}`, {
      method: 'PATCH',
      headers: { ...headers, Prefer: 'return=minimal' },
    ...
    if (!totalRes.ok) {
      ...
      return jsonError('Error al actualizar total de descuentos: ' + err, 500, request);
    }
...
export async function handleDevolucionParcialDespacho(request, env) {
  ...
  try {
    ...
    const cotizacionItems = cotItemsRes.ok ? ((await cotItemsRes.json()) || []) : [];
    const descuentosRows = discountsRes.ok ? ((await discountsRes.json()) || []) : [];
    const descuentoPorCotizacionItem = Object.fromEntries(
      descuentosRows.map(row => [row.cotizacion_item_id, Number(row.monto_usd || 0)])
    );
    ...
    for (const it of items) {
      ...
      if (isExchange) {
        ...
        // El descuento aplicado al despacho vive en despacho_descuentos y no
        // en descuento_pct del snapshot. El reembolso debe usar el precio neto
        // real; de lo contrario una devolución posterior vuelve a inflar CxC.
        const cotItem = cotizacionItems.find(item =>
          Number(item.orden) === Number(origItem.orden)
          && item.producto_id === origItem.producto_id
        ) || cotizacionItems.find(item => item.producto_id === origItem.producto_id);
        const descuentoLinea = cotItem ? Number(descuentoPorCotizacionItem[cotItem.id] || 0) : 0;
        const cantidadOriginal = Number(origItem.cantidad_original || origItem.cantidad || 0);
        const descuentoUnitario = cantidadOriginal > 0 ? descuentoLinea / cantidadOriginal : 0;
        const precioBaseNeto = Number(origItem.precio_unit_usd) * (1 - Number(origItem.descuento_pct || 0) / 100);
        const unitPriceAfterDiscount = Math.max(0, precioBaseNeto - descuentoUnitario);
        ...
        processedItems.push({
          ...
          cantidad_devuelta: qty,
          precio_unit_usd: unitPriceAfterDiscount,
          total_devuelto_usd: lineTotalRefund,
