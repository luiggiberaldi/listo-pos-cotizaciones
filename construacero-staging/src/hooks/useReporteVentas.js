...
export function useReporteVentas({ from, to, prevFrom, prevTo }) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let items = []
      if (cotIds.length > 0) {
        // Supabase .in() tiene límite, dividir en batches de 50 — en paralelo
        const batches = []
      ...
      // comisión (vendedor que vendió + dueño del cliente), cada una con su
      // vendedorid y porcentaje. Usamos las filas reales del Worker una por
      // beneficiario; solo estimamos cuando el despacho no tiene filas guardadas.
      ...
      comisiones.forEach(c => {
        ...
        // Filas split (0.5%/1.5% por cliente ajeno) van aparte: no son 2% ni 3%.
        if (c.es_split) {
