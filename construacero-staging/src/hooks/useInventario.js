...
import { buildSmartFilter, smartSearchProductos } from '../utils/smartSearch'
...
export function useInventario({ busqueda = '', categoria = '', page = 0, pageSize = 100, mostrarInactivos = false } = {}) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Para catálogos completos descargamos el universo permitido y aplicamos
      // el mismo motor fuzzy local que usan los autocompletados. El endpoint
      // híbrido está limitado a 100 filas y podía truncar búsquedas de inventario.
      if (busqueda.trim() !== '' && !esCatalogoCompleto) {
        try {
          ...
          // Si el híbrido devuelve cero por falta de embeddings, typos o una
          // firma de RPC antigua, dejamos continuar al filtro léxico local.
          if ((data.productos || []).length > 0) {
            return { productos: data.productos, totalCount: data.totalCount || data.productos.length }
          }
        } catch (error) {
      ...
      if (esPrivilegiado) {
        ...
        const productosResultado = esCatalogoCompleto && busqueda.trim()
          ? smartSearchProductos(productos, busqueda).map(({ _score, _match, _coverage, ...producto }) => producto)
          : productos
        const totalResultado = esCatalogoCompleto && busqueda.trim()
          ? productosResultado.length
          : totalCount

        return { productos: productosResultado, totalCount: totalResultado, truncado: totalCount > productos.length }
      }

      // Vendedor: RPC segura
      const fetchPagina = async (limit, offset) => {
        const { data, error } = await supabase.rpc('obtener_productos_vendedor', {
          p_busqueda: esCatalogoCompleto ? '' : busqueda.trim(),
      ...
      let productos = rows.map(({ total_count: _total_count, ...rest }) => rest)
      ...
      if (esCatalogoCompleto && totalCount > productos.length) {
        ...
        for (const extraRows of extras) {
          productos = productos.concat(extraRows.map(({ total_count: _total_count, ...rest }) => rest))
        }
      }
...
export function useCrearProducto() {
  ...
  return useMutation({
    mutationFn: async (campos) => {
      const { data, error } = await supabase.rpc('crear_producto_con_kardex_staging', {
...
export function useActualizarProducto() {
  ...
  return useMutation({
    mutationFn: async ({ id, campos, imagen_url }) => {
      const { data, error } = await supabase.rpc('actualizar_producto_con_kardex_staging', {
        p_id:           id,
        p_codigo:       campos.codigo?.trim()      || null,
        p_nombre:       campos.nombre.trim(),
        p_descripcion:  campos.descripcion?.trim() || null,
...
export function useBorrarProducto() {
  ...
  return useMutation({
    mutationFn: async (id) => {
      const { error } = await supabase.rpc('borrar_producto_con_kardex_staging', {
        p_producto_id: id,
      })
      if (error) throw error
    },
