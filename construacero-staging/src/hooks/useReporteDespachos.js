    ...
    queryFn: async () => {
      ...
      let q = supabase
        ...
        .select(`
          ...
          creado_en, despachada_en, entregada_en, entregada_en_ajustada,
          cliente_id,
      ...
      const inicioPeriodo = new Date(`${from}T00:00:00${tzStr}`).getTime()
      const finPeriodo = new Date(`${to}T23:59:59.999${tzStr}`).getTime()
      let lista = (despachos || [])
        .filter(d => {
          const fechaBase = d.entregada_en_ajustada || d.entregada_en || d.despachada_en || d.creado_en
          const timestamp = new Date(fechaBase || '').getTime()
          return Number.isFinite(timestamp) && timestamp >= inicioPeriodo && timestamp <= finPeriodo
        })
        .map(d => d.estado === 'entregada'
          ? { ...d, entregada_en_original: d.entregada_en, entregada_en: d.entregada_en_ajustada || d.entregada_en }
          : d
        )
        .sort((a, b) => {
          const aDate = a.entregada_en_ajustada || a.entregada_en || a.despachada_en || a.creado_en
          const bDate = b.entregada_en_ajustada || b.entregada_en || b.despachada_en || b.creado_en
