...
export default memo(function DespachoCard({ despacho, onCambiarEstado, onAnular, onReciclar, tasa = 0, config = {}, estadoCambiando = false, stockCheckData = undefined, stockCheckPending = false }) {
  ...
  const canEntregar = (['logistica', 'jefe', 'desarrollador', 'administracion'].includes(perfil?.rol)) && despacho.estado === 'despachada'
    ...
    <div className="group bg-white rounded-2xl border border-slate-200 hover:shadow-lg transition-all duration-200 flex flex-col">
      ...
      <div className="relative shrink-0 flex flex-col gap-1.5 px-3 py-2 rounded-t-2xl overflow-hidden"
          ...
          <div className="flex items-center gap-2">
            ...
            {esCreditoVenta && (
              <span className="bg-amber-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded border border-amber-400/50 shadow-sm uppercase tracking-wider leading-none shrink-0 select-none"
                ...
                PENDIENTE{pendientePagoUsd > 0 ? ` ${fmtMonto(pendientePagoUsd)}` : ''}
              </span>
            )}
            {esContadoVenta && (
        ...
        <div className="relative z-10 flex items-center flex-wrap gap-1.5">
          ...
          {tieneNotaObs && (
            <span
              className={`inline-flex items-center gap-1 bg-yellow-100 text-yellow-700 text-[9px] font-black px-1.5 py-0.5 rounded border border-yellow-300/70 shadow-sm uppercase tracking-wider leading-none ${puedeEditarNota ? 'cursor-pointer hover:bg-yellow-200' : 'select-none'}`}
              title={notaObsTexto}
