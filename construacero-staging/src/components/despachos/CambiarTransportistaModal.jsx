        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div className="space-y-2">
            ...
            {esLocal && (
              <div className="mt-2 p-2.5 rounded-xl bg-amber-50 border border-amber-200 text-[11px] text-amber-800">
                ...
                {!estadoDestino ? (
                  <span className="block mt-0.5 text-red-700 font-semibold">Falta el estado de destino; si no hay dirección alternativa, completa el estado del cliente.</span>
                ) : esFueraCarabobo && fleteNum > 0 ? (
                  <span className="block mt-0.5 text-amber-900 font-semibold">→ Comisión externa: <span className="font-black">${netoPreview.toFixed(2)}</span></span>
                ) : (
                  <span className="block mt-0.5 text-amber-900 font-semibold">Carabobo → nómina externa, no se liquida aquí.</span>
                )}
              </div>
            )}
          ...
          {esLocal && fleteNum > 0 && (
            <div className="space-y-2 pt-2 border-t border-slate-100">
              <p className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5"><MapPin size={13} /> Estado de destino requerido</p>
              ...
              <p className="text-[10px] text-slate-500">Si no se indica una dirección alternativa, se usará el estado del cliente.</p>
            </div>
          )}
