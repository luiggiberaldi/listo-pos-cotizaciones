...
const TABS = [
  ...
  { id: 'fiscal',        label: 'Fiscal',         icon: Receipt,   roles: ['administracion', 'desarrollador', 'jefe'] },
  { id: 'comisiones',    label: 'Comisiones',     icon: DollarSign, roles: ['administracion', 'desarrollador', 'jefe'] },
  { id: 'transportistas', label: 'Transportistas', icon: Truck,    roles: ['administracion', 'desarrollador', 'jefe'] },
...
function ComisionesTab({ campos, cambiar, isLoading, cargando }) {
  ...
  // Split por cliente ajeno: días activos (lista '0-6' separada por comas; vacío = todos)
...
// ─── FiscalTab: IVA, plantilla nota entrega, venta anticipada ────────────────
function FiscalTab({ campos, cambiar, isLoading, cargando }) {
  const disabled = isLoading || cargando
  return (
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-amber-700 mt-1 max-w-xl">
              Permite entregar productos cuando la mercancía ya está físicamente en el almacén pero aún no se ha cargado el ingreso al sistema. Los egresos se registran como <span className="font-mono bg-amber-100 px-1 py-0.5 rounded font-semibold text-amber-900">venta_anticipada</span> en el Kardex.
            </p>
          </div>
          <Toggle
            checked={!!campos.permitir_stock_negativo}
            onChange={v => cambiar('permitir_stock_negativo', v)}
            disabled={disabled}
            color="amber"
...
// ─── TransportistasTab: config global de choferes locales ───────────────────
function TransportistasTab({ campos, cambiar, isLoading, cargando }) {
  const disabled = isLoading || cargando
  const tipo = campos.transp_tipo_calculo === 'fija' ? 'fija' : 'porcentaje'
  const pct = Number(campos.transp_pct_comision) || 0
  const tarifa = Number(campos.transp_tarifa_fija_usd) || 0
  const fleteEj = 1000
  const netoPreview = tipo === 'fija'
    ? Math.min(tarifa, fleteEj)
    : Math.round(fleteEj * pct / 100 * 100) / 100

  return (
    <div className="space-y-5">
      <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/80 p-5 space-y-5 shadow-sm hover:border-slate-300/80 transition-all duration-300">
        <SectionHeader icon={Truck}>Transportistas Locales</SectionHeader>
        <p className="text-xs text-slate-500 -mt-2">
          Configuración global aplicable a todos los choferes marcados como <strong>locales</strong> en su ficha. El sistema calcula el neto a pagar al chofer por cada despacho y lo acumula como saldo pendiente para liquidación FIFO.
        </p>

        {/* Modo de cálculo — segmented control */}
        <div className="space-y-2">
          <label className="block text-xs font-semibold text-slate-500 mb-1.5 uppercase tracking-wide">
            Modo de cálculo del neto
          </label>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <button type="button" disabled={disabled}
              onClick={() => cambiar('transp_tipo_calculo', 'porcentaje')}
              className={`flex items-center gap-2 p-3 rounded-xl border text-left transition-all ${
                tipo === 'porcentaje'
                  ? 'border-amber-400 bg-amber-50 ring-2 ring-amber-200 shadow-sm'
                  : 'border-slate-200 bg-white hover:bg-slate-50'
              }`}>
              <Percent size={14} className={tipo === 'porcentaje' ? 'text-amber-600' : 'text-slate-400'} />
              <div>
                <div className="text-xs font-bold text-slate-800">Porcentaje del flete</div>
                <div className="text-[10px] text-slate-500">El chofer cobra un % del flete cobrado al cliente.</div>
              </div>
            </button>
            <button type="button" disabled={disabled}
              onClick={() => cambiar('transp_tipo_calculo', 'fija')}
              className={`flex items-center gap-2 p-3 rounded-xl border text-left transition-all ${
                tipo === 'fija'
                  ? 'border-amber-400 bg-amber-50 ring-2 ring-amber-200 shadow-sm'
                  : 'border-slate-200 bg-white hover:bg-slate-50'
              }`}>
              <DollarSign size={14} className={tipo === 'fija' ? 'text-amber-600' : 'text-slate-400'} />
              <div>
                <div className="text-xs font-bold text-slate-800">Tarifa fija por despacho</div>
                <div className="text-[10px] text-slate-500">Monto fijo en USD, acotado al flete cobrado.</div>
              </div>
            </button>
          </div>
        </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-baseline gap-3">
              <input type="number" min="0" max="100" step="0.01"
                ...
                className="w-24 px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-300 text-right font-bold shadow-inner"
                disabled={disabled} />
              <span className="text-lg font-bold text-amber-500">%</span>
              <p className="text-xs text-slate-500 font-semibold ml-2">
                Ej. flete $1,000 → neto <strong>{fmtUsd(netoPreview)}</strong>
              </p>
            </div>
          </div>
        ) : (
          <div className="space-y-2">
            ...
            <div className="flex items-baseline gap-3">
              ...
              <input type="number" min="0" step="0.01"
                ...
                className="w-28 px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-300 text-right font-bold shadow-inner"
                disabled={disabled} />
              <p className="text-xs text-slate-500 font-semibold ml-2">
                Ej. flete $1,000 → neto <strong>{fmtUsd(netoPreview)}</strong> (MIN tarifa, flete)
              </p>
            </div>
          </div>
        )}

        <div className="bg-amber-50/60 border border-amber-200/60 rounded-xl p-3 text-xs text-amber-800">
          <Info size={12} className="inline mr-1" />
          Para que un chofer aparezca en el reporte y reciba neto, marca la casilla <strong>«Es transportista local»</strong> en su ficha (sección Transportistas). Aplica <strong>todos</strong> los choferes locales a la vez; no se configura por chofer.
        </div>
      </div>
    </div>
  )
...
export default function ConfiguracionView() {
  ...
  useEffect(() => {
    if (config && Object.keys(config).length > 0) {
      setCampos({
        ...
        _comision_ext_extras:       config._comision_ext_extras       ?? [],        descuento_personal_pct:     config.descuento_personal_pct     ?? 10.00,
        comision_split_activo:      config.comision_split_activo      ?? true,
        ...
        transp_pct_comision:        config.transp_pct_comision        ?? 80,
  ...
  const esTabForm = tab === 'comisiones' || tab === 'negocio' || tab === 'fiscal' || tab === 'transportistas'
    ...
    <div className="p-3 sm:p-4 md:p-5 lg:p-6 max-w-4xl space-y-4">
      ...
      {esTabForm && (
        <form onSubmit={handleGuardar} className="space-y-5">
          ...
          {tab === 'fiscal' && <FiscalTab campos={campos} cambiar={cambiar} isLoading={isLoading} cargando={cargando} />}
          {tab === 'transportistas' && <TransportistasTab campos={campos} cambiar={cambiar} isLoading={isLoading} cargando={cargando} />}
