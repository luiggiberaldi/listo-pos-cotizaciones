...
- Decisión:
  ...
  7. **Una comisión, dos fechas — cuál manda**: la comisión tiene fecha de *despacho* (`notas_despacho.creado_en`) y fecha de *creación de la fila* (`comisiones.creadoen`). Con la tranca COD v4 pueden separarse días. **La fecha del despacho manda para todo**: filtro de cortes semanales (política 182), agrupación del PDF y columna FECHA de la tabla "Comisiones generadas" (`ModalDetalleVendedor.jsx`, commit 845916c). La fecha de creación de la fila es metadata de auditoría, no criterio de negocio. Caso fundacional: #3099 (despacho 10-sep, comisión 11-sep al conciliar el COD).
- Consecuencias:
