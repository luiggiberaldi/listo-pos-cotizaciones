...
| `271_staging_notas_titulo.sql` | Guía de despacho | — | ✅ Confirmado aplicado (columna `notas_titulo` + backfill 'nota'; staging 1,317 filas) | DROP COLUMN | Postflight interno + smoke PDF 3/3 | Título editable de la observación; principal vía release 11 |
| `272_staging_fix_saldo_favor_consumo.sql` | Finanzas/saldo a favor | — | ✅ Confirmado aplicado (5 RPCs con `consumo_credito` en fórmula de `saldo_a_favor`; postflight interno) | `272_rollback.sql` (cuerpos originales) | Arnés T0–T7 22/22 + E2E 123/123 | Causa raíz del fantasma Antoplast $112 (principal); reversiones no escriben la columna |
| `273_staging_entrega_sin_duplicar_saldo_favor.sql` | Finanzas/saldo a favor | — | ✅ Confirmado aplicado (guard anti-duplicado `IN ('abono','consumo_credito')` en 2 RPCs de entrega; postflight interno) | Cuerpos post-272 en `tmp/273/*.live.sql` | Arnés T2 (sin abono duplicado) + E2E 123/123 | La entrega ya no duplica el descuento cuando el Worker ya registró el consumo |
| `supabase/release/main/12_saldo_favor_consumo_review.sql` | Finanzas/saldo a favor | ✅ Confirmado aplicado (preflight intacto 4/4, apply OK, postflight byte-idéntico 4/4; guarda LCS +2/-1 y +1/-0; reparación: Antoplast $112→0 y ROSA SANCHEZ $4.14→0; smoke 9/9 sobre DES-2896/2697) | ✅ Espejo validado (272/273 vivas; dry-run savepoint OK) | `12_rollback_review.sql` (4 cuerpos originales del dump) | Builder + verify-diff + dry-run + apply + repair + smoke 2026-09-09 | Espejo del fix de staging; cierra la ventana de doble gasto de saldo a favor |
| `supabase/release/main/10_delegador_238b_review.sql` | Comisiones/ruta de entrega | ✅ Confirmado aplicado (postflight 100% verde: delegador vivo len=253, legacy fuera, 238b intacta, RPC entrega intacta; prueba de fuego #2975: NULL + 0 filas) | ✅ Dry-run transaccional en staging (paridad: delegador vivo desde 257) | `10_rollback_review.sql` (cuerpo legacy preservado) | Dry-run → apply → postflight + cleanup #2975 2026-09-07 | Cierra el agujero: la entrega usaba el calculador legacy sin tranca COD ni split sábados |
...
> Última revisión documental: 2026-09-09. El estado de ejecución solo se marca ✅ cuando una sesión de la `BITACORA.md` lo confirmó con verificación post-apply (postflight, smoke o E2E). Todo lo demás queda ❓ — **no inventar estado**.
